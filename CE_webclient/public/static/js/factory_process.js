...
//! display ONE product process contents: material use, emission, byproducts, utilities
function display_a_product_process_details(product_line_info) {
    ...
    fill_model_basis_table(product_line_info.process_basis, product_info);
...
function fill_table_content(a_table, info) {
    ...
    if (info.length > 0) {
        ...
        $table.append('<thead><tr class="table-info"><th>名称</th><th>数值每年(单位:'+ quantity_unit +')</th><th>单位成本(' + currency_value_per_unit + ')</th><th>年成本(单位:'+ currency +')</th></tr></thead>');
//        $table.children('tr:first').addClass('table-info');
        var tblbody = $table.get(0).appendChild(document.createElement('tbody'));
        for (var i = 0; i < info.length; ++i) {
            var an_item = info[i];
            add_a_table_row(tblbody, [an_item.name, an_item.quantity, an_item.value_per_unit, an_item.annual_value], [false, false, true, false]);
        }
    }
}
...
function fill_model_basis_table(process_basis_info, product_info) {
    ...
    add_a_table_row(tblbody, ["单位成本", product_info.value_per_unit, product_info.currency_value_per_unit], [false, true, false]);
...
function product_process_change_handler(factory_id, rf_id, data) {
    ...
    if (product_line_info.length == 1)
        display_a_product_process_details(product_line_info[0][1]);
}